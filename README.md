...
file [types.py](./python/sm_aicli/types.py), the rest of the project contains implementing
algorithms. The central role is assigned to the **Conversation** abstraction.

The main loop of the program arbitrates **Actors** adding utterances on top of the stack of the
already existing ones. The whole design mimics Free Monad evaluation, where **Utterance** plays the
role of the Free Monad itself. Most of the monad constructors are modeled as flags of the
**Intention** part of the Utterance. Using these flags, an actor can request bringing more actors
to the conversation.
...
User-facing terminal actor implements the same API producing
utterances in the cource of interpretation of input language. The language parser is generated by
the Lark libary from the pre-defined [grammar](#commands-overview).

Each actor gets a read-only view of Conversation, determines the related Utterance and then takes
responsibility of encoding it into the appropriate third-party format, computing the response, and
encoding it back into the Utterance.  For example, a popular choice is the
`{'role':'system'|'assistant'|'user', 'content': str}` structure used by OpenAI API.
